...
bool XmlPullParser::escapeString(string &value) {
 ...
 std::string XmlPullParser::get(const Dictionary &d, const std::string &key, const std::string &def ) const {
    auto it = d.find(key) ;
...
string XmlPullParser::getAttribute(const string &name, const string &val) const {
    return get(attributes_, name, val) ;
}
