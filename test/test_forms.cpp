...
int main(int argc, char *argv[]) {
    ...
    app.addRoute("GET|POST", "/form", [&rdr, &form, &translator](HTTPServerRequest& req, HTTPServerResponse& resp) {
        ...
        if ( req.getMethod() == "POST" && form.process(req.getPostAttributes()) ) {
            resp.write(translator.translate("welcome", locale->locale(), Variant::Object{{"name", form.getValue("username")}})) ;
            return ;
         } 
           
        try {
           
            resp.write(rdr.render("test_form.html", Variant::Object{{"form", form.render(&translator, locale->locale())}})) ;
